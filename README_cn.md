...
渲染一张专用画面，由操作员通过 **AP** 与 **BOOT** 两个按键给出"通过/失败"判定。
...
* 上电后自动初始化所有外设驱动
* 顺序运行 10 项硬件测试，每项之间阻塞等待操作员判定（`AP = OK/PASS`，`BOOT = FAIL`）
